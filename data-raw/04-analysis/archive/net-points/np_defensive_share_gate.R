...
build <- function(s, blame, spread, ball_winner_share = NULL) {
  ...
  if (!is.null(ball_winner_share)) args$ball_winner_share <- ball_winner_share
...
arm <- function(label, blame, spread = "matchup", bws = NULL) {
  a25 <- build(2025L, blame, spread, bws)
...
# lower blame = more of the turnover paid to the defence that won the ball
grid <- list(
  list("blame 0.50 (least def)", 0.50, "matchup", NULL),
  list("blame 0.30 (shipped)",   0.30, "matchup", NULL),
  list("blame 0.15",             0.15, "matchup", NULL),
  list("blame 0.00 (most def)",  0.00, "matchup", NULL),
  list("blame 0.15, context",    0.15, "context", NULL),
  list("blame 0.15, tog",        0.15, "tog", NULL),
  list("blame 0.15, def acts",   0.15, "defensive_acts", NULL),
  list("blame 0.15, winner .85", 0.15, "matchup", 0.85),
  list("blame 0.30, winner .85", 0.30, "matchup", 0.85))

out <- rbindlist(lapply(grid, function(g) {
  r <- arm(g[[1]], g[[2]], g[[3]], g[[4]])
...
print(out[order(r)])
fwrite(out, file.path(S, "np_defensive_share_gate.csv"))
