...
test_that("data objects have correct structure", {
  ...
  data(results, package = "torp", envir = environment())
  expect_true(is.data.frame(results))
  expect_true(nrow(results) > 0)
  
  data(teams, package = "torp", envir = environment())
  expect_true(is.data.frame(teams))
  expect_true(nrow(teams) > 0)
})
