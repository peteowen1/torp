  ...
  for (p in seq_len(n_players)) {
    ...
    for (g in seq_len(games_per_player)) {
      rows[[length(rows) + 1]] <- data.frame(
        ...
        avail_only = FALSE,
        bounces = rpois(1, 0.3),
        def_half_pressure_acts = rpois(1, 6),
        centre_clearances = rpois(1, if (pos_idx == 2) 2 else 0),
        stoppage_clearances = rpois(1, 1),
        effective_kicks = rpois(1, 7),
        effective_disposals = rpois(1, 14),
        intercept_marks = rpois(1, if (pos_idx == 1) 2 else 0),
        f50_ground_ball_gets = rpois(1, 1),
        score_launches = rpois(1, 1),
        marks_on_lead = rpois(1, 0.3),
        tackles_inside50 = rpois(1, 0.5),
        kickins = rpois(1, if (pos_idx == 1) 2 else 0),
        centre_bounce_attendances = rpois(1, if (pos_idx == 4) 15 else if (pos_idx == 2) 5 else 0),
        ruck_contests = rpois(1, if (pos_idx == 4) 25 else 0),
        contest_def_one_on_ones = rpois(1, 0.5),
        contest_off_one_on_ones = rpois(1, 0.5),
        contest_off_wins = rpois(1, 0.1),
        contest_def_losses = rpois(1, 0.1),
...
test_that("raw averages are per-player, not league-wide", {
  ...
  dt <- data.table::data.table(
    ...
    hitouts = 0, hitouts_to_advantage = 0,
    clearances = 3, contested_marks = 1,
    ground_ball_gets = 3, marks_inside50 = 1,
    rebound50s = 2, metres_gained = 300, spoils = 1,
    intercepts = 2, one_percenters = 2, pressure_acts = 10,
