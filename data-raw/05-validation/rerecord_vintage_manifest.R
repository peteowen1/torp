...
#
# Run it BEFORE the code that widened the constants reaches `main`, which is the
# order the guard enforces (see NEWS 1.8.1, the v7 promotion).
#
...
say("re-recording vintage ", VERSION, " (torp ", utils::packageVersion("torp"), ")")
...
# after the last overwrite (recorded in NEXT-STEPS, 2026-09-09). Poll the
# UNCACHED url read_ratings_manifest() actually fetches rather than assuming.
...
for (i in 1:20) {
  ...
  n_leaves <- length(unlist(a8$defining_constants))
  say("  attempt ", i, ": torp_version=", a8$torp_version,
      "  leaves=", n_leaves, "  canonical=", after$canonical)
  if (n_leaves == leaves_live) { ok <- TRUE; break }
}
...
say("leaves:        ", length(unlist(a8$defining_constants)), paste0(" (was ", leaves_before, ")"))
