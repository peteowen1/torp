...
mdl_wk <- function(df,weeknum){
  val <- paste0("CD_M2022014",sprintf('%02d',weeknum))
  ...
  if (!is.na(max(team_func_df$homeTeamScore.matchScore.totalScore))) {
    ...
    #########
    test_df <- df %>% filter(season.x == 2022, round.roundNumber.x.x == weeknum, team_type_fac == "home")
...
#resultz <- purrr::map_df(1:30,~mdl_wk(team_mdl_df,.))
resultz <- furrr::future_map_dfr(1:30,~mdl_wk(team_mdl_df,.),.options = furrr_options(seed=TRUE))
rez <- resultz  %>% filter(!is.na(bits))
####
#test_rez <- rez %>% filter(season.x == 2022)
library(MLmetrics)
