...
match_xgs <- function(season = get_afl_season(), round = get_afl_week(), match_id = NA, quarter = 1:4) {
  ...
  shots_df <- df %>%
    ...
    dplyr::summarise(
      ...
      home_xscore = sum(dplyr::if_else(team == home_team_team_name, xscore*shot_row, 0), na.rm = T),
      ...
      away_xscore = sum(dplyr::if_else(team == away_team_team_name, xscore*shot_row, 0), na.rm = T),
