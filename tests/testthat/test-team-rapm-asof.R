...
test_that("fit_team_spm_asof: returns NULL with a LOUD warning when ref_date has no strictly-prior season to train on", {
  local_mocked_bindings(
    load_player_game_data = function(seasons, ...) .mock_pgd_dated(seasons),
    load_player_stats = function(seasons, comp = "AFLM") .mock_ps_stats_dated(seasons),
    load_results = function(seasons, comp = "AFLM") .mock_results_dated(seasons)
  )
  ref_date <- min(.mock_results_dated(2024L)$utc_start_time)  # earliest available season
  ...
  expect_warning(
    ...
    "no strictly-prior season"
  )
  expect_null(result)
})

test_that("fit_team_spm_asof: trains only on seasons strictly before ref_date's season", {
  ...
  ref_date <- min(.mock_results_dated(2024L)$utc_start_time)  # first match of 2024
  fake_ratings <- data.table::data.table(
  ...
  expect_equal(unique(result$spm_train_seasons_max), 2023L)
})
