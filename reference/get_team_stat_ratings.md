...
Aggregates player-level stat rating estimates to team level. For each
team, sums and averages the Bayesian stat rating estimates of its
current players.

## Usage

``` r
get_team_stat_ratings(team_name = NULL, top_n = 22)
...
- top_n:
  ...
  Maximum number of players per team to include (ordered by total stat
  rating). Default 22 (a full team).
