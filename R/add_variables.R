...
#' verified across the full 2026 season, \code{team_id_mdl} matches the next
#' row's \code{team_id_mdl} (the side that resolves the contest) on 12,250 of
#' 12,267 stoppage rows (99.86%). Reproduces the worked example that
#' surfaced this (\code{CD_M20260140608}, \code{Ball Up Call} then
#' \code{Gather From Hitout}): \code{team_id_mdl} is Brisbane's on both,
#' \code{exp_pts} -0.532 then -0.532, i.e. one consistent frame throughout,
#' not a sign flip mid-stoppage.
#'
...
#' torpdata and inthegame-blog, PLUS the published "How EPV works" post,
