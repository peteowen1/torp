...
plyr_gm_df <-
  ...
  dplyr::summarise(
    ...
    disp_credits = sum(dplyr::if_else(pos_team == -1, delta_epv - 0.04, delta_epv + 0.08) / 2),
    disp_credits_wt = sum((dplyr::if_else(pos_team == -1, delta_epv - 0.04, delta_epv + 0.08) * max(weight_gm)) / 2),
    disp_wpa = sum((wpa) / 2),
  ...
  dplyr::left_join(
    model_data_wp %>% # add_wp_vars() %>%
      ...
      dplyr::summarise(
        recv_credits = sum(dplyr::if_else(pos_team == -1, (1.5 * delta_epv * pos_team) + 0.1, (1 * delta_epv * pos_team) + 0.05) / 2),
        recv_credits_wt = sum((dplyr::if_else(pos_team == -1, (1.5 * delta_epv * pos_team) + 0.1, ((1 * delta_epv * pos_team) + 0.05)) * max(weight_gm)) / 2),
        recv_wpa = sum((wpa) / 2),
        receptions = dplyr::n()
      ),
  ...
  dplyr::mutate(
    recv_credits = tidyr::replace_na(recv_credits, 0), # + 0.15 * effective_disposals - bounces * 0.5,
    recv_credits_wt = tidyr::replace_na(recv_credits_wt, 0),
    disp_credits = tidyr::replace_na(disp_credits, 0) - (bounces * 0.2),
    disp_credits_wt = tidyr::replace_na(disp_credits_wt, 0),
    spoil_credits = tidyr::replace_na(spoil_credits, 0),
    spoil_credits_wt = tidyr::replace_na(spoil_credits_wt, 0),
    hitout_credits = tidyr::replace_na(hitout_credits, 0),
    hitout_credits_wt = tidyr::replace_na(hitout_credits_wt, 0),
    total_credits = recv_credits + disp_credits + spoil_credits + hitout_credits,
    total_credits_wt = recv_credits_wt + disp_credits_wt + spoil_credits_wt + hitout_credits_wt,
  ...
  dplyr::mutate(
    # total_credits_adj = total_credits - quantile(total_credits, 0.3, na.rm = T),
    recv_credits_adj = recv_credits - quantile(recv_credits, 0.4, na.rm = T),
    disp_credits_adj = disp_credits - quantile(disp_credits, 0.4, na.rm = T),
    spoil_credits_adj = spoil_credits - quantile(spoil_credits, 0.4, na.rm = T),
    hitout_credits_adj = hitout_credits - quantile(hitout_credits, 0.4, na.rm = T),
    total_credits_adj = recv_credits_adj + disp_credits_adj + spoil_credits_adj + hitout_credits_adj
  ) %>%
  ungroup() %>%
  mutate(season = season.x) %>%
  dplyr::relocate(total_credits_adj, disposals_pbp, recv_credits_adj, disp_credits_adj, spoil_credits_adj, hitout_credits_adj) %>%
