...
# ############ 2021
# #############################
# torp_df_21 <- purrr::map_df(1:27, ~torp_ratings(2021, .)) %>% mutate(row_id = paste0(player_id,season,sprintf('%02d',round)))
#
# ############# 2022
# torp_df_22 <- purrr::map_df(1:27, ~ torp_ratings(2022, .)) %>% mutate(row_id = paste0(player_id,season,sprintf('%02d',round)))
#
# ########### 2023
# torp_df_23 <- purrr::map_df(1:get_afl_week(type = 'next'), ~ torp_ratings(2023, .)) %>% mutate(row_id = paste0(player_id,season,sprintf('%02d',round)))
#
# ## final
